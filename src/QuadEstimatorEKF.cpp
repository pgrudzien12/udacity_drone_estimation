...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
	...
	Quaternion<float> qCurrent = Quaternion<float>::FromEuler123_RPY(rollEst, pitchEst, ekfState(6));
	Quaternion<float> qDot = qCurrent.IntegrateBodyRate(gyro, dtIMU);

	// I'm not sure if this is needed but seems I image that it may avoid a drift
	qDot.Normalise();

	V3D eulerAngles = qDot.ToEulerRPY();

	float predictedRoll = eulerAngles.x;
	float predictedPitch = eulerAngles.y;
	ekfState(6) = eulerAngles.z; // yaw
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
	...
	predictedState(5) = curState(5) + (acc_inertial.z - CONST_GRAVITY) * dt; // *not including gravity*
	predictedState(6) = curState(6);// (I suppose that) this does not neeed to be integrated
...
MatrixXf QuadEstimatorEKF::GetRbgPrime(float roll, float pitch, float yaw)
{
	...
	// Compute the trigonometric functions once to avoid redundancy
	float cr = cos(roll);
	float sr = sin(roll);
	float cp = cos(pitch);
	float sp = sin(pitch);
	...
	RbgPrime(1, 0) = -sr * sp * sy - cr * cy;
	RbgPrime(1, 1) = sr * sp * cy - cr * sy;
	RbgPrime(1, 2) = 0;

	RbgPrime(2, 0) = cr * sp * sy - sr * cy;
	RbgPrime(2, 1) = -cr * sp * cy - sr * sy;
	RbgPrime(2, 2) = 0;
...
void QuadEstimatorEKF::Predict(float dt, V3F accel, V3F gyro)
{
	...
	VectorXf accelGlobal = RbgPrime * u;
	gPrime(3, 6) = accelGlobal(0) * dt;
	gPrime(4, 6) = accelGlobal(1) * dt;
	gPrime(5, 6) = accelGlobal(2) * dt;

	MatrixXf ekfCovNew = gPrime * ekfCov * gPrime.transpose() + Q;
	ekfCov = ekfCovNew;
...
void QuadEstimatorEKF::UpdateFromGPS(V3F pos, V3F vel)
{
	...
	// zFromX is the predicted measurement based on the current state
	//zFromX(0) = ekfState(0);
	//zFromX(1) = ekfState(1);
	//zFromX(2) = ekfState(2);	
	//zFromX(3) = ekfState(3);	
...
void QuadEstimatorEKF::UpdateFromMag(float magYaw)
{
	...
	float predictedYaw = ekfState(6);

	float yawError = magYaw - predictedYaw;  

	zFromX(0) = ekfState(6);
	// ensure its between -pi and pi
	if (yawError > F_PI) 
		zFromX(0) += 2 * F_PI;
	if (yawError < -F_PI) 
		zFromX(0) -= 2 * F_PI;

	// Jacobian is only last elkement
	hPrime(0, 6) = 1.0f;
